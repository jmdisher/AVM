...
public class StringShadowingTest {
    ...
    public void testSingleString() {
        ...
        AionAddress dappAddr = new AionAddress(avm.run(kernel, new Transaction[] {tx}, ExecutionType.ASSUME_MAINCHAIN, kernel.getBlockNumber()-1)[0].getResult().copyOfTransactionOutput().get());
        kernel.generateBlock();
        ...
        Assert.assertTrue(java.util.Arrays.equals(new int[]{96354, 3, 1, -1}, new ABIDecoder(result.copyOfTransactionOutput().get()).decodeOneIntegerArray()));
        kernel.generateBlock();
        ...
        Assert.assertTrue(java.util.Arrays.equals(new boolean[]{true, false, true, false, true, false, false}, new ABIDecoder(result.copyOfTransactionOutput().get()).decodeOneBooleanArray()));
        kernel.generateBlock();

        txData = encodeNoArgsMethodCall("singleStringReturnChar");
        tx = AvmTransactionUtil.call(from, dappAddr, kernel.getNonce(from), BigInteger.ZERO, txData, energyLimit, energyPrice);
        result = avm.run(kernel, new Transaction[] {tx}, ExecutionType.ASSUME_MAINCHAIN, kernel.getBlockNumber()-1)[0].getResult();
        Assert.assertEquals('a', new ABIDecoder(result.copyOfTransactionOutput().get()).decodeOneCharacter());
        kernel.generateBlock();

        txData = encodeNoArgsMethodCall("singleStringReturnBytes");
        tx = AvmTransactionUtil.call(from, dappAddr, kernel.getNonce(from), BigInteger.ZERO, txData, energyLimit, energyPrice);
        result = avm.run(kernel, new Transaction[] {tx}, ExecutionType.ASSUME_MAINCHAIN, kernel.getBlockNumber()-1)[0].getResult();
        Assert.assertTrue(java.util.Arrays.equals(new byte[]{'a', 'b', 'c'}, new ABIDecoder(result.copyOfTransactionOutput().get()).decodeOneByteArray()));
        kernel.generateBlock();

        txData = encodeNoArgsMethodCall("singleStringReturnLowerCase");
        tx = AvmTransactionUtil.call(from, dappAddr, kernel.getNonce(from), BigInteger.ZERO, txData, energyLimit, energyPrice);
        result = avm.run(kernel, new Transaction[] {tx}, ExecutionType.ASSUME_MAINCHAIN, kernel.getBlockNumber()-1)[0].getResult();
        Assert.assertEquals("abc", new ABIDecoder(result.copyOfTransactionOutput().get()).decodeOneString());
        kernel.generateBlock();
        ...
        tx = AvmTransactionUtil.call(from, dappAddr, kernel.getNonce(from), BigInteger.ZERO, txData, energyLimit, energyPrice);
        result = avm.run(kernel, new Transaction[] {tx}, ExecutionType.ASSUME_MAINCHAIN, kernel.getBlockNumber()-1)[0].getResult();
        Assert.assertEquals("Sub", new ABIDecoder(result.copyOfTransactionOutput().get()).decodeOneString());
    ...
    public void testBatchingCalls() {
        ...
        txData = encodeNoArgsMethodCall("singleStringReturnChar");
        batch[2] = AvmTransactionUtil.call(from, dappAddr, kernel.getNonce(from).add(new BigInteger("2")), BigInteger.ZERO, txData, energyLimit, energyPrice);
        ...
        Assert.assertArrayEquals(new int[]{96354, 3, 1, -1}, new ABIDecoder(results[0].getResult().copyOfTransactionOutput().get()).decodeOneIntegerArray());
        Assert.assertArrayEquals(new boolean[]{true, false, true, false, true, false, false}, new ABIDecoder(results[1].getResult().copyOfTransactionOutput().get()).decodeOneBooleanArray());
        Assert.assertEquals('a', new ABIDecoder(results[2].getResult().copyOfTransactionOutput().get()).decodeOneCharacter());
        Assert.assertArrayEquals(new byte[]{'a', 'b', 'c'}, new ABIDecoder(results[3].getResult().copyOfTransactionOutput().get()).decodeOneByteArray());
        Assert.assertEquals("abc", new ABIDecoder(results[4].getResult().copyOfTransactionOutput().get()).decodeOneString());
        Assert.assertEquals("ABC", new ABIDecoder(results[5].getResult().copyOfTransactionOutput().get()).decodeOneString());
